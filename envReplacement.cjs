...
const derivedEnvName = isResettingToPlaceholders || !process.argv[2] ? "test" : process.argv[2];
let placeholderPrinterString = "";

process.on("exit", (code) => {
  if (code == 101) {
    console.info("Please pass --force flag after env name, to continue building even with the missing placeholders");
    console.info("For example, you can run:");
    console.info(" node envReplacement.cjs {YourEnvName} --force");
  }
  else if (code == 500) {
    console.error(`${emoji.emojify(":smiling_face_with_tear:", onMissingEmoji)} Internal script error mentioned above!`);
  }
