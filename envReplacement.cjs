...
readFile("./public/index.html", "utf-8", function(err, contents) {
  ...
  if (derivedEnvName !== "test") {
    if (
      JSON.stringify(Object.keys(envConfig["test"]).sort()) !==
      JSON.stringify(Object.keys(envConfig[derivedEnvName]).sort())
    ) {
      console.error(
        `${emoji.emojify(
          ":smiling_face_with_tear:",
          onMissingEmoji
        )} Someone has messed up, and the placeholders in "test" environment and "${derivedEnvName}" environment is not same, Please check envConfig.json file`
      );
      process.exit(400);
    }
  }
  ...
  Object.keys(envConfig[derivedEnvName]).map((placeholder) => {
    ...
    if (isResettingToPlaceholders) {
      ...
      replaced = latestContents.replace(
        envConfig[derivedEnvName][placeholder],
        placeholder
      );
    } else {
  ...
  placeholderPrinterString = `${
    copiedMissingPlaceholdersArray.length
      ? copiedMissingPlaceholdersArray.join(", ")
      : ""
  }${copiedMissingPlaceholdersArray.length ? ", " : ""}${
    poppedArrElem ? `${poppedArrElem}` : ""
  }`;
  ...
  if (isFroced) {
    ...
    if (missingPlaceholdersArray.length) {
      console.info(
        "I hope you are aware what you are doing by passing the flag --force."
      );
      console.info(
    ...
    // else we will just tell them, to calm down and not use force flag, as it did not actually force anything
      ...
      console.info(
        `Your --force flag did not actually force the node script to execute, looks like you have placed all the placeholders perfectly ${emoji.emojify(
          ":white_check_mark:",
          onMissingEmoji
        )}.`
      );
    ...
    if (missingPlaceholdersArray.length) {
      console.error(
        `${emoji.emojify(
          ":x:",
          onMissingEmoji
        )} The placeholder ${placeholderPrinterString} was missing from your index.html file.`
      );
      process.exit(101);
    }
  }
  ...
  writeFile("./public/index.html", replaced, "utf-8", function(err) {
    if (err) {
      ...
      if (isResettingToPlaceholders) {
        console.log(
          "With grace of almighty, env restorement to placeholders happened successfully"
        );
      } else {
        console.log(
          "With grace of almighty, env replacement happened successfully"
        );
      }
    }
  });
